    ...
    bcrypt.hash(req.body.password1, salt, (err, hash) => {
      ...
      database("users")
        ...
        .then(user => {
          ...
          let link = "http://tsukiru.ddns.net:3000/api/users/verify/" + user[0].token;
        ...
        database
          ...
          .then(result => {
            if (result.length) {
              ...
              let link =
                "http://tsukiru.ddns.net:3000/api/users/verify/" + result[0].token;
        ...
        bcrypt.compare(req.body.password, data[0].password).then(isMatch => {
          if (isMatch) {
            ...
            jwt.sign(
              ...
              { expiresIn: 3600 },
              (err, token) => {
                res.status(200).json({"token" : "Bearer " + token, "id" :payload.id});
              }
            );
          } else {
      ...
      if (emailData.length == 0) {
        ...
        database
          ...
          .then(done => {
            ...
            let link = "http://tsukiru.ddns.net:3000/api/users/verify/" + resetToken;
